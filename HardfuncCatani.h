#ifndef HardfuncCatani
#define HardfuncCatani


void qqCatani(ExVariables exVars, ExVariables extrafo, std::complex<dd_real> H[70], std::complex<dd_real> Fr[29], std::complex<dd_real> sqrtA[3], 
		dd_real mu, dd_real nlight, dd_real Ncol, dd_real &H0qq, dd_real &H1qqmu, dd_real &H2qqmu){
    ...
    H1qqmu=Ncol*qqH1nf0mu+nlight*qqH1nf1mu;
    H2qqmu=Ncol*Ncol*qqH2nf0mu+nlight*Ncol*qqH2nf1mu+nlight*nlight*qqH2nf2mu;
...
void qxqCatani(ExVariables extest, ExVariables extrafoperm, ExVariables extrafopermconj, std::complex<dd_real> Hperm[70], std::complex<dd_real> Frperm[29], std::complex<dd_real> sqrtB[3], dd_real mu, dd_real nlight, dd_real Ncol, dd_real &H0qxq, dd_real &H1qxqmu, dd_real &H2qxqmu){
    ...
    H1qxqmu=Ncol*qxqH1nf0mu+nlight*qxqH1nf1mu;
    H2qxqmu=Ncol*Ncol*qxqH2nf0mu+nlight*Ncol*qxqH2nf1mu+nlight*nlight*qxqH2nf2mu;

}

void ggCatani(ExVariables exVars, ExVariables extrafo, ExVariables extrafoperm, ExVariables extrafopermconj, std::complex<dd_real> H[70], std::complex<dd_real> Hperm[70], std::complex<dd_real> Fr[29], std::complex<dd_real> Frperm[29], std::complex<dd_real> sqrtA[3], std::complex<dd_real> sqrtB[3], 
		dd_real mu, dd_real nlight, dd_real Ncol, dd_real &H0ggx,  dd_real &H0gxg, dd_real &H1ggxmu, dd_real &H1gxgmu, dd_real &H2ggmu){
    ...
    H0ggx=2*sqggxtree.real(); //we need the 2*H0 for the different permutations
    H0gxg=2*sqgxgtree.real(); //they contributes in a different way in the change of IR scheme
    dd_real H0gg=H0ggx+H0gxg;
    
    dd_real zeta3=dd_real(1.2020569031595942853997381615114499);
    dd_real pi=dd_real(3.1415926535897932384626433832795);
    
    dd_real ggxH1nf0mu=2*sqggxolnf0.real() + dd_real(31.0)/dd_real(6.0) * H0ggx * log(mu*mu);
    dd_real ggxH1nf1mu=2*sqggxolnf1.real() - dd_real(2.)/dd_real(3.) * H0ggx * log(mu*mu);
    
    dd_real gxgH1nf0mu=2*sqgxgolnf0.real() + dd_real(31.0)/dd_real(6.0) * H0gxg * log(mu*mu);
    dd_real gxgH1nf1mu=2*sqgxgolnf1.real() - dd_real(2.)/dd_real(3.) * H0gxg * log(mu*mu);

    dd_real ggH1nf0mu=ggxH1nf0mu+gxgH1nf0mu;
    dd_real ggH1nf1mu=ggxH1nf1mu+gxgH1nf1mu;
    ...
    std::cout << "ggH2nf0mu=" << ggH2nf0mu << std::endl;
    std::cout << "ggH2nf1mu=" << ggH2nf1mu << std::endl;
    std::cout << "ggH2nf2mu=" << ggH2nf2mu << std::endl;

    H1ggxmu=Ncol*ggxH1nf0mu+nlight*ggxH1nf1mu;
    H1gxgmu=Ncol*gxgH1nf0mu+nlight*gxgH1nf1mu;
    
    H2ggmu=Ncol*Ncol*ggH2nf0mu+nlight*Ncol*ggH2nf1mu+nlight*nlight*ggH2nf2mu;
